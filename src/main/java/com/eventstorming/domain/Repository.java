...
public interface {{namePascalCase}}Repository extends PagingAndSortingRepository<{{namePascalCase}}, {{aggregateRoot.keyFieldDescriptor.className}}>{
	{{#attached 'ReadModel' this}}
  @Query(value = "select {{camelCase ../name}} " +
        "from {{pascalCase ../name}} {{camelCase ../name}} " +
        "where {{#queryParameters}}(:{{camelCase name}} is null or {{camelCase ../../name}}.{{camelCase name}} like %:{{camelCase name}}%) and {{/queryParameters}}true")
	List<{{pascalCase ../name}}> findBy{{pascalCase name}}({{#queryParameters}}{{{className}}} {{camelCase name}}, {{/queryParameters}}Pageable pageable);
	{{/attached}}
}
//>>> PoEAA / Repository
<function>
